...
Это содержимое вставляется с помощью перезаписи файлов, поэтому авторы могут добавлять в созданную документацию по API любой объем содержимого в формате Markdown.
...
В коде ниже показано, как вызвать метод записи с помощью пакета Azure SDK для .NET. 
